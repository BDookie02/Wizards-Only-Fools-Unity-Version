...
if (-not $resolvedBuildRoot.StartsWith('D:\', [System.StringComparison]::OrdinalIgnoreCase) -or
    ...
    throw 'Chicago city capture paths must stay on D:.'
}
...
public static class WofChicagoCityCapture {
...
'@
[WofChicagoCityCapture]::SetProcessDPIAware() | Out-Null

$executable = Join-Path $resolvedBuildRoot 'WizardsOnlyFools.exe'
$logRoot = Join-Path $resolvedOutputRoot 'logs'
$profileRoot = Join-Path $resolvedOutputRoot ('chicago-city-capture-profile-' + [Guid]::NewGuid().ToString('N'))
$logPath = Join-Path $logRoot 'chicago-city-capture.log'
$capturePath = Join-Path $resolvedOutputRoot 'chicago-city-desktop.png'
...
[System.IO.File]::WriteAllText(
    ...
    '{"version":1,"playerName":"Chicago QA","questUnlockedSpells":["blink"],"spellQuestAssignments":[],"questFlags":[]}')
...
$arguments = @(
    ...
    '--wof-solo', '--wof-chicago-city-view-probe', '--wof-auto-exit=90',
...
try {
    ...
    do {
        ...
        $ready = (Test-Path -LiteralPath $logPath -PathType Leaf) -and
            (Select-String -LiteralPath $logPath -Pattern 'CHICAGO_CITY_SCENE_READY' -Quiet)
    } while (-not $ready -and -not $process.HasExited -and [DateTime]::UtcNow -lt $deadline)
    if (-not $ready) { throw 'Chicago city view probe did not become ready.' }
    ...
    if ($windowHandle -eq [IntPtr]::Zero) { throw 'Chicago city player has no main window.' }
    [WofChicagoCityCapture]::ShowWindowAsync($windowHandle, 9) | Out-Null
    [WofChicagoCityCapture]::SetForegroundWindow($windowHandle) | Out-Null
    Start-Sleep -Milliseconds 2600

    $rect = New-Object WofChicagoCityCapture+RECT
    if (-not [WofChicagoCityCapture]::GetClientRect($windowHandle, [ref]$rect)) { throw 'GetClientRect failed.' }
    $point = New-Object WofChicagoCityCapture+POINT
    if (-not [WofChicagoCityCapture]::ClientToScreen($windowHandle, [ref]$point)) { throw 'ClientToScreen failed.' }
