...
if (-not $resolvedBuildRoot.StartsWith('D:\', [System.StringComparison]::OrdinalIgnoreCase) -or
    ...
    throw 'Desert controller probe paths must stay on D:.'
}
...
$captureRoot = Join-Path $resolvedOutputRoot 'desert-controller'
$profileRoot = Join-Path $resolvedOutputRoot ('desert-controller-profile-' + [Guid]::NewGuid().ToString('N'))
$playerTempRoot = Join-Path $resolvedOutputRoot 'player-temp'
$logPath = Join-Path $logRoot 'desert-controller-runtime.log'
$captureNames = @(
    'desert-route-south-road.png',
    'desert-route-well-detour.png',
    'desert-route-north-gate.png',
    'desert-route-expansion-chunk.png'
)
...
[System.IO.File]::WriteAllText(
    ...
    '{"version":1,"playerName":"Desert Controller QA","questUnlockedSpells":["blink"],"spellQuestAssignments":[],"questFlags":[]}')
...
$arguments = @(
    ...
    '--wof-solo', '--wof-desert-village-view-probe', "--wof-desert-controller-probe=$captureRoot",
    '--wof-auto-exit=120', "--wof-profile-root=$profileRoot", '-logFile', $logPath
)
...
try {
    ...
    do {
        ...
        if (Test-Path -LiteralPath $logPath -PathType Leaf) {
            $complete = Select-String -LiteralPath $logPath -SimpleMatch 'DESERT_CONTROLLER_PROBE_COMPLETE' -Quiet
            $failed = Select-String -LiteralPath $logPath -SimpleMatch 'DESERT_CONTROLLER_PROBE_FAILED' -Quiet
        }
    ...
    if (-not $complete) {
        ...
        throw "Desert native-controller probe did not complete.`n$tail"
    }
    ...
    foreach ($captureName in $captureNames) {
        ...
        if (-not (Test-Path -LiteralPath $capturePath -PathType Leaf) -or
            ...
            throw "Desert controller probe did not create $capturePath"
        }
    ...
    [PSCustomObject]@{
        Complete = $complete
        Receipt = (Select-String -LiteralPath $logPath -SimpleMatch 'DESERT_CONTROLLER_PROBE_COMPLETE' |
