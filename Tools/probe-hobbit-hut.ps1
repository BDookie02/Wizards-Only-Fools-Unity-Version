param(
    ...
    [int]$Height = 720,
    [ValidateSet('plains', 'jungle', 'mushroom')]
...
if (-not $resolvedBuildRoot.StartsWith('D:\', [System.StringComparison]::OrdinalIgnoreCase) -or
    ...
    throw 'Hobbit-hut probe paths must stay on D:.'
}
...
$profileRoot = Join-Path $resolvedOutputRoot ('hobbit-hut-probe-profile-' + [Guid]::NewGuid().ToString('N'))
...
public static class WofHobbitHutCapture {
...
'@
[WofHobbitHutCapture]::SetProcessDPIAware() | Out-Null

$executable = Join-Path $resolvedBuildRoot 'WizardsOnlyFools.exe'
$logPath = Join-Path $logRoot "hobbit-hut-${Variant}-probe.log"
$capturePath = Join-Path $resolvedOutputRoot "hobbit-hut-${Variant}-${Width}x${Height}.png"
if (-not (Test-Path -LiteralPath $executable -PathType Leaf)) {
    throw "Windows player not found: $executable"
}
[System.IO.File]::WriteAllText(
    (Join-Path $profileRoot 'survival-save-v1.json'),
    '{"version":1,"playerName":"Hobbit Hut QA","questUnlockedSpells":["blink"],"spellQuestAssignments":[],"questFlags":[]}')
...
$arguments = @(
    ...
    '--wof-solo', "--wof-hobbit-hut-probe=$Variant", '--wof-sky-probe=day', '--wof-auto-exit=90',
...
try {
    ...
    do {
        ...
        $ready = (Test-Path -LiteralPath $logPath -PathType Leaf) -and
            (Select-String -LiteralPath $logPath -SimpleMatch 'HOBBIT_HUT_PROBE_READY' -Quiet)
    } while (-not $ready -and -not $process.HasExited -and [DateTime]::UtcNow -lt $deadline)
    if (-not $ready) { throw "Hobbit-hut $Variant probe did not become ready." }
    ...
    if ($windowHandle -eq [IntPtr]::Zero) { throw 'Hobbit-hut player has no main window.' }
    [WofHobbitHutCapture]::ShowWindowAsync($windowHandle, 9) | Out-Null
    [WofHobbitHutCapture]::SetWindowPos($windowHandle, [IntPtr](-1), 0, 0, 0, 0, 0x53) | Out-Null
    [WofHobbitHutCapture]::SetForegroundWindow($windowHandle) | Out-Null

    $rect = New-Object WofHobbitHutCapture+RECT
    if (-not [WofHobbitHutCapture]::GetClientRect($windowHandle, [ref]$rect)) { throw 'GetClientRect failed.' }
    $point = New-Object WofHobbitHutCapture+POINT
    if (-not [WofHobbitHutCapture]::ClientToScreen($windowHandle, [ref]$point)) { throw 'ClientToScreen failed.' }
    ...
    [WofHobbitHutCapture]::SetWindowPos($windowHandle, [IntPtr](-2), 0, 0, 0, 0, 0x53) | Out-Null
    ...
    $runtimeFailure = Select-String -LiteralPath $logPath `
        ...
        -Quiet
    if ($runtimeFailure) { throw "Hobbit-hut player logged a runtime exception: $logPath" }

    [PSCustomObject]@{
        Capture = $capturePath
        Log = $logPath
        Ready = (Select-String -LiteralPath $logPath -SimpleMatch 'HOBBIT_HUT_PROBE_READY' |
    ...
    if (-not $process.HasExited) {
        [WofHobbitHutCapture]::SetWindowPos($process.MainWindowHandle, [IntPtr](-2), 0, 0, 0, 0, 0x53) | Out-Null
