...
if (-not $resolvedBuildRoot.StartsWith('D:\', [System.StringComparison]::OrdinalIgnoreCase) -or
    ...
    throw 'Swamp controller probe paths must stay on D:.'
}
...
$captureRoot = Join-Path $resolvedOutputRoot 'swamp-controller'
$profileRoot = Join-Path $resolvedOutputRoot ('swamp-controller-profile-' + [Guid]::NewGuid().ToString('N'))
$playerTempRoot = Join-Path $resolvedOutputRoot 'player-temp'
$logPath = Join-Path $logRoot 'swamp-controller-runtime.log'
$captureNames = @(
    'swamp-route-north-ramp.png',
    'swamp-route-central-platform.png',
    'swamp-route-east-dock.png',
    'swamp-route-east-ramp-exit.png'
)
...
$arguments = @(
    ...
    '--wof-solo', '--wof-swamp-village-view-probe', "--wof-swamp-traversal-probe=$captureRoot",
...
try {
    ...
    do {
        ...
        if (Test-Path -LiteralPath $logPath -PathType Leaf) {
            $complete = Select-String -LiteralPath $logPath -SimpleMatch 'SWAMP_TRAVERSAL_PROBE_COMPLETE' -Quiet
            $failed = Select-String -LiteralPath $logPath -SimpleMatch 'SWAMP_TRAVERSAL_PROBE_FAILED' -Quiet
        }
    ...
    if (-not $complete) {
        ...
        throw "Swamp native-controller probe did not complete.`n$tail"
    }
    ...
    foreach ($captureName in $captureNames) {
        ...
        if (-not (Test-Path -LiteralPath $capturePath -PathType Leaf) -or
            ...
            throw "Swamp controller probe did not create $capturePath"
        }
    ...
    [PSCustomObject]@{
        Complete = $complete
        Receipt = (Select-String -LiteralPath $logPath -SimpleMatch 'SWAMP_TRAVERSAL_PROBE_COMPLETE' |
