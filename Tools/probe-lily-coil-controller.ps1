...
if (-not $resolvedBuildRoot.StartsWith('D:\', [System.StringComparison]::OrdinalIgnoreCase) -or
    ...
    throw 'Lily Coil controller probe paths must stay on D:.'
}
...
$captureRoot = Join-Path $resolvedOutputRoot 'lily-coil-controller'
$profileRoot = Join-Path $resolvedOutputRoot ('lily-coil-controller-profile-' + [Guid]::NewGuid().ToString('N'))
$playerTempRoot = Join-Path $resolvedOutputRoot 'player-temp'
$logPath = Join-Path $logRoot 'lily-coil-controller-runtime.log'
$captureNames = @(
    'lily-coil-traversal-lower-cap.png',
    'lily-coil-traversal-midpoint.png',
    'lily-coil-traversal-upper-cap.png'
)
...
[System.IO.File]::WriteAllText(
    ...
    '{"version":1,"playerName":"Lily Coil Controller QA","questUnlockedSpells":["blink"],"spellQuestAssignments":[],"questFlags":[]}')
...
$arguments = @(
    ...
    '--wof-solo', "--wof-lily-coil-controller-probe=$captureRoot", '--wof-auto-exit=150',
...
try {
    ...
    if ($process.MainWindowHandle -eq [IntPtr]::Zero) {
        throw 'Lily Coil controller probe has no main window.'
    }
    $shell = New-Object -ComObject WScript.Shell
    $shell.AppActivate($process.Id) | Out-Null

    $deadline = [DateTime]::UtcNow.AddSeconds(140)
    $complete = $false
    $failed = $false
    do {
        ...
        $process.Refresh()
        if (Test-Path -LiteralPath $logPath -PathType Leaf) {
            $complete = Select-String -LiteralPath $logPath -Pattern 'LILY_COIL_CONTROLLER_PROBE_COMPLETE nativeGamepad=true' -Quiet
            $failed = Select-String -LiteralPath $logPath -Pattern 'LILY_COIL_CONTROLLER_PROBE_FAILED' -Quiet
        }
    ...
    if (-not $complete) {
        ...
        throw "Lily Coil native-controller probe did not complete.`n$tail"
    }
    ...
    foreach ($captureName in $captureNames) {
        ...
        if (-not (Test-Path -LiteralPath $capturePath -PathType Leaf) -or
            ...
            throw "Lily Coil controller probe did not create $capturePath"
        }
    ...
    $marker = Select-String -LiteralPath $logPath -Pattern 'LILY_COIL_CONTROLLER_PROBE_COMPLETE' |
