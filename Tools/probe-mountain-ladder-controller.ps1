...
if (-not $resolvedBuildRoot.StartsWith('D:\', [System.StringComparison]::OrdinalIgnoreCase) -or
    ...
    throw 'Mountain ladder controller probe paths must stay on D:.'
}

$executable = Join-Path $resolvedBuildRoot 'WizardsOnlyFools.exe'
$logRoot = Join-Path $resolvedOutputRoot 'logs'
$profileRoot = Join-Path $resolvedOutputRoot ('mountain-ladder-controller-profile-' + [Guid]::NewGuid().ToString('N'))
$playerTempRoot = Join-Path $resolvedOutputRoot 'player-temp'
$logPath = Join-Path $logRoot 'mountain-ladder-controller-runtime.log'
...
[System.IO.File]::WriteAllText(
    ...
    '{"version":1,"playerName":"Mountain Ladder Controller QA","questUnlockedSpells":["blink"],"spellQuestAssignments":[],"questFlags":[]}')
...
$arguments = @(
    ...
    '--wof-solo', '--wof-mountain-ladder-controller-probe', '--wof-auto-exit=90',
...
try {
    ...
    if ($process.MainWindowHandle -eq [IntPtr]::Zero) {
        throw 'Mountain ladder controller probe has no main window.'
    }
    ...
    do {
        ...
        if (Test-Path -LiteralPath $logPath -PathType Leaf) {
            $complete = Select-String -LiteralPath $logPath -Pattern 'MOUNTAIN_LADDER_CONTROLLER_PROBE_COMPLETE leftStick=true ladder=3:0-mineshaft-ladder-0 deltaY=[1-9][0-9.]* climbSpeed=8.4 damping=0.22' -Quiet
            $failed = Select-String -LiteralPath $logPath -Pattern 'MOUNTAIN_LADDER_CONTROLLER_PROBE_FAILED' -Quiet
        }
    ...
    if (-not $complete) {
        ...
        throw "Mountain ladder native-controller probe did not complete.`n$tail"
    }
    $marker = Select-String -LiteralPath $logPath -Pattern 'MOUNTAIN_LADDER_CONTROLLER_PROBE_COMPLETE' | Select-Object -Last 1 -ExpandProperty Line
