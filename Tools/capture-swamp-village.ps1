...
public static class WofSwampVillageCapture {
...
'@
[WofSwampVillageCapture]::SetProcessDPIAware() | Out-Null

$executable = Join-Path $resolvedBuildRoot 'WizardsOnlyFools.exe'
$logRoot = Join-Path $resolvedOutputRoot 'logs'
$profileRoot = Join-Path $resolvedOutputRoot ('swamp-village-capture-profile-' + [Guid]::NewGuid().ToString('N'))
$logPath = Join-Path $logRoot 'swamp-village-capture.log'
$capturePath = Join-Path $resolvedOutputRoot 'swamp-village-desktop.png'
...
[System.IO.File]::WriteAllText(
    ...
    '{"version":1,"playerName":"Swamp QA","questUnlockedSpells":["blink"],"spellQuestAssignments":[],"questFlags":[]}')
...
$arguments = @(
    ...
    '--wof-solo', '--wof-swamp-village-view-probe', '--wof-auto-exit=90',
...
try {
    ...
    do {
        ...
        $ready = (Test-Path -LiteralPath $logPath -PathType Leaf) -and
            (Select-String -LiteralPath $logPath -Pattern 'SWAMP_VILLAGE_SCENE_READY' -Quiet)
    } while (-not $ready -and -not $process.HasExited -and [DateTime]::UtcNow -lt $deadline)
    if (-not $ready) { throw 'Swamp village view probe did not become ready.' }
    ...
    if ($windowHandle -eq [IntPtr]::Zero) { throw 'Swamp village player has no main window.' }
    [WofSwampVillageCapture]::ShowWindowAsync($windowHandle, 9) | Out-Null
    [WofSwampVillageCapture]::SetForegroundWindow($windowHandle) | Out-Null
    Start-Sleep -Milliseconds 3200

    $rect = New-Object WofSwampVillageCapture+RECT
    if (-not [WofSwampVillageCapture]::GetClientRect($windowHandle, [ref]$rect)) { throw 'GetClientRect failed.' }
    $point = New-Object WofSwampVillageCapture+POINT
    if (-not [WofSwampVillageCapture]::ClientToScreen($windowHandle, [ref]$point)) { throw 'ClientToScreen failed.' }
    ...
    [PSCustomObject]@{ Capture=$capturePath; Log=$logPath; Width=$width; Height=$height; Bytes=(Get-Item -LiteralPath $capturePath).Length }
}
finally {
