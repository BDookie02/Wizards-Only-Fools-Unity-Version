...
if (-not $resolvedBuildRoot.StartsWith('D:\', [System.StringComparison]::OrdinalIgnoreCase) -or
    ...
    throw 'Generic quest controller probe paths must stay on D:.'
}

$executable = Join-Path $resolvedBuildRoot 'WizardsOnlyFools.exe'
$logRoot = Join-Path $resolvedOutputRoot 'logs'
$profileRoot = Join-Path $resolvedOutputRoot ('generic-quest-controller-profile-' + [Guid]::NewGuid().ToString('N'))
$logPath = Join-Path $logRoot 'generic-quest-controller-runtime.log'
...
$arguments = @(
    ...
    '0',
    '--wof-solo',
    '--wof-villager-view-probe',
    '--wof-generic-quest-controller-probe',
    '--wof-auto-exit=90',
...
try {
    ...
    if ($process.MainWindowHandle -eq [IntPtr]::Zero) {
        throw 'Generic quest controller probe has no main window.'
    }
    ...
    do {
        ...
        $process.Refresh()
        if (Test-Path -LiteralPath $logPath -PathType Leaf) {
            $complete = Select-String -LiteralPath $logPath -Pattern 'GENERIC_QUEST_CONTROLLER_PROBE_COMPLETE interact=true.*assignment=(?!none)\S+.*messages=3' -Quiet
            $failed = Select-String -LiteralPath $logPath -Pattern 'GENERIC_QUEST_CONTROLLER_PROBE_FAILED' -Quiet
        }
    ...
    if (-not $complete) {
        ...
        throw "Generic quest native-controller probe did not complete.`n$tail"
    }

    $marker = Select-String -LiteralPath $logPath -Pattern 'GENERIC_QUEST_CONTROLLER_PROBE_COMPLETE interact=true.*assignment=(?!none)\S+.*messages=3' |
