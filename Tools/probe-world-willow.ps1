...
if (-not $resolvedBuildRoot.StartsWith('D:\', [System.StringComparison]::OrdinalIgnoreCase) -or
    ...
    throw 'World-willow probe paths must stay on D:.'
}
...
$profileRoot = Join-Path $resolvedOutputRoot ('world-willow-probe-profile-' + [Guid]::NewGuid().ToString('N'))
...
public static class WofWorldWillowCapture {
...
'@
[WofWorldWillowCapture]::SetProcessDPIAware() | Out-Null

$executable = Join-Path $resolvedBuildRoot 'WizardsOnlyFools.exe'
$logPath = Join-Path $logRoot "world-willow-${WillowIndex}-probe.log"
$firstPath = Join-Path $resolvedOutputRoot "world-willow-${WillowIndex}-first-${Width}x${Height}.png"
$secondPath = Join-Path $resolvedOutputRoot "world-willow-${WillowIndex}-second-${Width}x${Height}.png"
[System.IO.File]::WriteAllText(
    (Join-Path $profileRoot 'survival-save-v1.json'),
    '{"version":1,"playerName":"World Willow QA","questUnlockedSpells":["blink"],"spellQuestAssignments":[],"questFlags":[]}')
...
$arguments = @(
    ...
    '--wof-solo', "--wof-world-willow-probe=$WillowIndex", '--wof-sky-probe=day', '--wof-auto-exit=90',
...
try {
    ...
    do {
        ...
        $ready = (Test-Path -LiteralPath $logPath -PathType Leaf) -and
            (Select-String -LiteralPath $logPath -SimpleMatch 'WORLD_WILLOW_PROBE_POSITIONED' -Quiet) -and
            (Select-String -LiteralPath $logPath -SimpleMatch 'SURVIVAL_WORLD_WILLOWS_READY' -Quiet)
    } while (-not $ready -and -not $process.HasExited -and [DateTime]::UtcNow -lt $deadline)
    if (-not $ready) { throw 'World-willow probe did not become ready.' }
    ...
    if ($windowHandle -eq [IntPtr]::Zero) { throw 'World-willow player has no main window.' }
    [WofWorldWillowCapture]::ShowWindowAsync($windowHandle, 9) | Out-Null
    [WofWorldWillowCapture]::SetForegroundWindow($windowHandle) | Out-Null

    $rect = New-Object WofWorldWillowCapture+RECT
    if (-not [WofWorldWillowCapture]::GetClientRect($windowHandle, [ref]$rect)) { throw 'GetClientRect failed.' }
    $point = New-Object WofWorldWillowCapture+POINT
    if (-not [WofWorldWillowCapture]::ClientToScreen($windowHandle, [ref]$point)) { throw 'ClientToScreen failed.' }
    ...
    do {
        ...
        $passed = (Test-Path -LiteralPath $logPath -PathType Leaf) -and
            (Select-String -LiteralPath $logPath -SimpleMatch 'WORLD_WILLOW_PROBE_PASS' -Quiet)
    } while (-not $passed -and -not $process.HasExited -and [DateTime]::UtcNow -lt $passDeadline)
    if (-not $passed) { throw 'World-willow particle motion did not pass in the executable.' }
    ...
    $runtimeFailure = Select-String -LiteralPath $logPath `
        ...
        -Quiet
    if ($runtimeFailure) { throw "World-willow player logged a runtime exception: $logPath" }
    ...
    if ($firstHash -eq $secondHash) { throw 'World-willow captures are identical; motion was not visually represented.' }

    [PSCustomObject]@{
        First = $firstPath
        Second = $secondPath
        Log = $logPath
        Ready = (Select-String -LiteralPath $logPath -SimpleMatch 'WORLD_WILLOW_PROBE_POSITIONED' |
            Select-Object -Last 1).Line
        Passed = (Select-String -LiteralPath $logPath -SimpleMatch 'WORLD_WILLOW_PROBE_PASS' |
            Select-Object -Last 1).Line
        Width = $capturedWidth
