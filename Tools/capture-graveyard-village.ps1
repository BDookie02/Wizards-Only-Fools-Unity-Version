param(
    ...
    [ValidateSet('exterior', 'interior', 'tombs', 'fence')]
...
if (-not $resolvedBuildRoot.StartsWith('D:\', [System.StringComparison]::OrdinalIgnoreCase) -or
    ...
    throw 'Graveyard village capture paths must stay on D:.'
}
...
public static class WofGraveyardVillageCapture {
...
'@
[WofGraveyardVillageCapture]::SetProcessDPIAware() | Out-Null

$executable = Join-Path $resolvedBuildRoot 'WizardsOnlyFools.exe'
$logRoot = Join-Path $resolvedOutputRoot 'logs'
$profileRoot = Join-Path $resolvedOutputRoot ("graveyard-$View-capture-profile-" + [Guid]::NewGuid().ToString('N'))
$logPath = Join-Path $logRoot "graveyard-$View-capture.log"
$capturePath = Join-Path $resolvedOutputRoot "graveyard-$View-desktop.png"
...
[System.IO.File]::WriteAllText(
    ...
    '{"version":1,"playerName":"Graveyard QA","questUnlockedSpells":["blink"],"spellQuestAssignments":[],"questFlags":[]}')
foreach ($target in @($logPath, $capturePath)) {
    if (Test-Path -LiteralPath $target -PathType Leaf) { Remove-Item -LiteralPath $target -Force }
}

$probeArgument = if ($View -eq 'exterior') {
    '--wof-graveyard-village-view-probe'
} else {
    "--wof-graveyard-village-view-probe=$View"
}
$arguments = @(
...
$positionedMarker = "GRAVEYARD_VILLAGE_PROBE_POSITIONED variant=$View"
...
try {
    ...
    do {
        ...
        $ready = (Test-Path -LiteralPath $logPath -PathType Leaf) -and
            ...
            (Select-String -LiteralPath $logPath -SimpleMatch 'GRAVEYARD_VILLAGE_SCENE_READY' -Quiet)
    } while (-not $ready -and -not $process.HasExited -and [DateTime]::UtcNow -lt $deadline)
    if (-not $ready) { throw "Graveyard village $View view probe did not become ready." }
    ...
    if ($windowHandle -eq [IntPtr]::Zero) { throw 'Graveyard village player has no main window.' }
    [WofGraveyardVillageCapture]::ShowWindowAsync($windowHandle, 9) | Out-Null
    [WofGraveyardVillageCapture]::SetForegroundWindow($windowHandle) | Out-Null
    Start-Sleep -Milliseconds 4200

    $rect = New-Object WofGraveyardVillageCapture+RECT
    if (-not [WofGraveyardVillageCapture]::GetClientRect($windowHandle, [ref]$rect)) { throw 'GetClientRect failed.' }
    $point = New-Object WofGraveyardVillageCapture+POINT
    if (-not [WofGraveyardVillageCapture]::ClientToScreen($windowHandle, [ref]$point)) { throw 'ClientToScreen failed.' }
