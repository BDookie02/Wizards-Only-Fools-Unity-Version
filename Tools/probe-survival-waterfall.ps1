...
if (-not $resolvedBuildRoot.StartsWith('D:\', [System.StringComparison]::OrdinalIgnoreCase) -or
    ...
    throw 'Waterfall probe paths must stay on D:.'
}
...
$profileRoot = Join-Path $resolvedOutputRoot ('waterfall-probe-profile-' + [Guid]::NewGuid().ToString('N'))
...
public static class WofWaterfallCapture {
...
'@
[WofWaterfallCapture]::SetProcessDPIAware() | Out-Null

$executable = Join-Path $resolvedBuildRoot 'WizardsOnlyFools.exe'
$logPath = Join-Path $logRoot 'survival-waterfall-probe.log'
$capturePath = Join-Path $resolvedOutputRoot "survival-waterfall-${Width}x${Height}.png"
if (-not (Test-Path -LiteralPath $executable -PathType Leaf)) {
    throw "Windows player not found: $executable"
}
[System.IO.File]::WriteAllText(
    (Join-Path $profileRoot 'survival-save-v1.json'),
    '{"version":1,"playerName":"Waterfall QA","questUnlockedSpells":["blink"],"spellQuestAssignments":[],"questFlags":[]}')
...
$arguments = @(
    ...
    '--wof-solo', '--wof-waterfall-probe', '--wof-sky-probe=day', '--wof-auto-exit=90',
...
try {
    ...
    do {
        ...
        $ready = (Test-Path -LiteralPath $logPath -PathType Leaf) -and
            (Select-String -LiteralPath $logPath -SimpleMatch 'WATERFALL_PROBE_READY' -Quiet)
    } while (-not $ready -and -not $process.HasExited -and [DateTime]::UtcNow -lt $deadline)
    if (-not $ready) { throw 'Waterfall probe did not become ready.' }
    ...
    if ($windowHandle -eq [IntPtr]::Zero) { throw 'Waterfall player has no main window.' }
    [WofWaterfallCapture]::ShowWindowAsync($windowHandle, 9) | Out-Null
    [WofWaterfallCapture]::SetWindowPos($windowHandle, [IntPtr](-1), 0, 0, 0, 0, 0x53) | Out-Null
    [WofWaterfallCapture]::SetForegroundWindow($windowHandle) | Out-Null

    $rect = New-Object WofWaterfallCapture+RECT
    if (-not [WofWaterfallCapture]::GetClientRect($windowHandle, [ref]$rect)) { throw 'GetClientRect failed.' }
    $point = New-Object WofWaterfallCapture+POINT
    if (-not [WofWaterfallCapture]::ClientToScreen($windowHandle, [ref]$point)) { throw 'ClientToScreen failed.' }
    ...
    [WofWaterfallCapture]::SetWindowPos($windowHandle, [IntPtr](-2), 0, 0, 0, 0, 0x53) | Out-Null
    ...
    $runtimeFailure = Select-String -LiteralPath $logPath `
        ...
        -Quiet
    if ($runtimeFailure) { throw "Waterfall player logged a runtime exception: $logPath" }

    [pscustomobject]@{
        Capture = $capturePath
        Log = $logPath
        Ready = (Select-String -LiteralPath $logPath -SimpleMatch 'WATERFALL_PROBE_READY' |
    ...
    if (-not $process.HasExited) {
        [WofWaterfallCapture]::SetWindowPos($process.MainWindowHandle, [IntPtr](-2), 0, 0, 0, 0, 0x53) | Out-Null
