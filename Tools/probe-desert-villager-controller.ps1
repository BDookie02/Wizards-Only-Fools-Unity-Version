...
if (-not $resolvedBuildRoot.StartsWith('D:\', [System.StringComparison]::OrdinalIgnoreCase) -or
    ...
    throw 'Desert villager controller probe paths must stay on D:.'
}

$executable = Join-Path $resolvedBuildRoot 'WizardsOnlyFools.exe'
$logRoot = Join-Path $resolvedOutputRoot 'logs'
$profileRoot = Join-Path $resolvedOutputRoot ('desert-villager-controller-profile-' + [Guid]::NewGuid().ToString('N'))
$logPath = Join-Path $logRoot 'desert-villager-controller-runtime.log'
...
$arguments = @(
    ...
    '--wof-solo', '--wof-desert-village-view-probe', '--wof-desert-villager-controller-probe',
...
try {
    ...
    if ($process.MainWindowHandle -eq [IntPtr]::Zero) {
        throw 'Desert villager controller probe has no main window.'
    }
    ...
    do {
        ...
        if (Test-Path -LiteralPath $logPath -PathType Leaf) {
            $complete = Select-String -LiteralPath $logPath -Pattern 'DESERT_VILLAGER_CONTROLLER_PROBE_COMPLETE interactX=true npc=4:-4-desert-building-0 town=survival-desert-villagers-4:-4 assignment=(?!none)\S+ messages=3' -Quiet
            $failed = Select-String -LiteralPath $logPath -Pattern 'DESERT_VILLAGER_CONTROLLER_PROBE_FAILED' -Quiet
        }
    ...
    if (-not $complete) {
        ...
        throw "Desert villager native-controller probe did not complete.`n$tail"
    }
    $marker = Select-String -LiteralPath $logPath -Pattern 'DESERT_VILLAGER_CONTROLLER_PROBE_COMPLETE' |
