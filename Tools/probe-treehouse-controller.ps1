...
if (-not $resolvedBuildRoot.StartsWith('D:\', [System.StringComparison]::OrdinalIgnoreCase) -or
    ...
    throw 'Tree-house controller probe paths must stay on D:.'
}
...
$captureRoot = Join-Path $resolvedOutputRoot 'treehouse-controller'
$profileRoot = Join-Path $resolvedOutputRoot ('treehouse-controller-profile-' + [Guid]::NewGuid().ToString('N'))
$playerTempRoot = Join-Path $resolvedOutputRoot 'player-temp'
$logPath = Join-Path $logRoot 'treehouse-controller-runtime.log'
$captureNames = @(
    'treehouse-spiral-start.png',
    'treehouse-spiral-midpoint.png',
    'treehouse-spiral-top.png',
    'treehouse-bridge-start.png',
    'treehouse-bridge-end.png'
)
...
[System.IO.File]::WriteAllText(
    ...
    '{"version":1,"playerName":"Tree House Controller QA","questUnlockedSpells":["blink"],"spellQuestAssignments":[],"questFlags":[]}')
...
$arguments = @(
    ...
    '--wof-solo', "--wof-treehouse-controller-probe=$captureRoot", '--wof-auto-exit=120',
...
try {
    ...
    do {
        ...
        $process.Refresh()
        if (Test-Path -LiteralPath $logPath -PathType Leaf) {
            $complete = Select-String -LiteralPath $logPath -SimpleMatch 'TREEHOUSE_CONTROLLER_PROBE_COMPLETE' -Quiet
            $failed = Select-String -LiteralPath $logPath -SimpleMatch 'TREEHOUSE_CONTROLLER_PROBE_FAILED' -Quiet
        }
    ...
    if (-not $complete) {
        ...
        throw "Tree-house native-controller probe did not complete.`n$tail"
    }
    ...
    foreach ($captureName in $captureNames) {
        ...
        if (-not (Test-Path -LiteralPath $capturePath -PathType Leaf) -or
            ...
            throw "Tree-house controller probe did not create $capturePath"
        }
    ...
    [PSCustomObject]@{
        Complete = $complete
        Spiral = (Select-String -LiteralPath $logPath -SimpleMatch 'TREEHOUSE_SPIRAL_TRAVERSAL_PASS' |
            Select-Object -Last 1 -ExpandProperty Line)
        Bridge = (Select-String -LiteralPath $logPath -SimpleMatch 'TREEHOUSE_BRIDGE_TRAVERSAL_PASS' |
            Select-Object -Last 1 -ExpandProperty Line)
        Log = $logPath
