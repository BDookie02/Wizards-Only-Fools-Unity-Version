...
if (-not $resolvedBuildRoot.StartsWith('D:\', [System.StringComparison]::OrdinalIgnoreCase) -or
    ...
    throw 'Swamp villager controller probe paths must stay on D:.'
}

$executable = Join-Path $resolvedBuildRoot 'WizardsOnlyFools.exe'
$logRoot = Join-Path $resolvedOutputRoot 'logs'
$profileRoot = Join-Path $resolvedOutputRoot ('swamp-villager-controller-profile-' + [Guid]::NewGuid().ToString('N'))
$playerTempRoot = Join-Path $resolvedOutputRoot 'player-temp'
$logPath = Join-Path $logRoot 'swamp-villager-controller-runtime.log'
...
[System.IO.File]::WriteAllText(
    ...
    '{"version":1,"playerName":"Swamp Controller QA","questUnlockedSpells":["blink"],"spellQuestAssignments":[],"questFlags":[]}')
...
$arguments = @(
    ...
    '--wof-solo', '--wof-swamp-village-view-probe', '--wof-swamp-villager-controller-probe',
...
try {
    ...
    if ($process.MainWindowHandle -eq [IntPtr]::Zero) { throw 'Swamp villager controller probe has no main window.' }
    ...
    do {
        ...
        if (Test-Path -LiteralPath $logPath -PathType Leaf) {
            $complete = Select-String -LiteralPath $logPath -Pattern 'SWAMP_VILLAGER_CONTROLLER_PROBE_COMPLETE interactX=true npc=0:-3-swamp-hut-0 town=survival-swamp-villagers-0:-3 assignment=(?!none)\S+ messages=3' -Quiet
            $failed = Select-String -LiteralPath $logPath -Pattern 'SWAMP_VILLAGER_CONTROLLER_PROBE_FAILED' -Quiet
        }
    ...
    if (-not $complete) {
        ...
        throw "Swamp villager native-controller probe did not complete.`n$tail"
    }
    $marker = Select-String -LiteralPath $logPath -Pattern 'SWAMP_VILLAGER_CONTROLLER_PROBE_COMPLETE' | Select-Object -Last 1 -ExpandProperty Line
