...
if (-not $resolvedBuildRoot.StartsWith('D:\', [System.StringComparison]::OrdinalIgnoreCase) -or
    ...
    throw 'Underbrush probe paths must stay on D:.'
}
...
$profileRoot = Join-Path $resolvedOutputRoot ('underbrush-probe-profile-' + [Guid]::NewGuid().ToString('N'))
...
public static class WofUnderbrushCapture {
...
'@
[WofUnderbrushCapture]::SetProcessDPIAware() | Out-Null

$executable = Join-Path $resolvedBuildRoot 'WizardsOnlyFools.exe'
$logPath = Join-Path $logRoot 'underbrush-probe.log'
$capturePath = Join-Path $resolvedOutputRoot "underbrush-${Variant}-${Width}x${Height}.png"
if (-not (Test-Path -LiteralPath $executable -PathType Leaf)) {
    throw "Windows player not found: $executable"
}
[System.IO.File]::WriteAllText(
    (Join-Path $profileRoot 'survival-save-v1.json'),
    '{"version":1,"playerName":"Underbrush QA","questUnlockedSpells":["blink"],"spellQuestAssignments":[],"questFlags":[]}')
...
$arguments = @(
    ...
    '--wof-solo', "--wof-underbrush-probe=$Variant", '--wof-sky-probe=day', '--wof-auto-exit=90',
...
try {
    ...
    do {
        ...
        $ready = (Test-Path -LiteralPath $logPath -PathType Leaf) -and
            (Select-String -LiteralPath $logPath -SimpleMatch 'UNDERBRUSH_PROBE_PASS' -Quiet)
    } while (-not $ready -and -not $process.HasExited -and [DateTime]::UtcNow -lt $deadline)
    if (-not $ready) { throw 'Underbrush probe did not become ready.' }
    ...
    if ($windowHandle -eq [IntPtr]::Zero) { throw 'Underbrush player has no main window.' }
    [WofUnderbrushCapture]::ShowWindowAsync($windowHandle, 9) | Out-Null
    [WofUnderbrushCapture]::SetWindowPos($windowHandle, [IntPtr](-1), 0, 0, 0, 0, 0x53) | Out-Null
    [WofUnderbrushCapture]::SetForegroundWindow($windowHandle) | Out-Null

    $rect = New-Object WofUnderbrushCapture+RECT
    if (-not [WofUnderbrushCapture]::GetClientRect($windowHandle, [ref]$rect)) { throw 'GetClientRect failed.' }
    $point = New-Object WofUnderbrushCapture+POINT
    if (-not [WofUnderbrushCapture]::ClientToScreen($windowHandle, [ref]$point)) { throw 'ClientToScreen failed.' }
    ...
    [WofUnderbrushCapture]::SetWindowPos($windowHandle, [IntPtr](-2), 0, 0, 0, 0, 0x53) | Out-Null
    ...
    $runtimeFailure = Select-String -LiteralPath $logPath `
        ...
        -Quiet
    if ($runtimeFailure) { throw "Underbrush player logged a runtime exception: $logPath" }

    [PSCustomObject]@{
        Capture = $capturePath
        Log = $logPath
        Ready = (Select-String -LiteralPath $logPath -SimpleMatch 'UNDERBRUSH_PROBE_POSITIONED' |
            Select-Object -Last 1).Line
        Passed = (Select-String -LiteralPath $logPath -SimpleMatch 'UNDERBRUSH_PROBE_PASS' |
            Select-Object -Last 1).Line
        Width = $capturedWidth
    ...
    if (-not $process.HasExited) {
        [WofUnderbrushCapture]::SetWindowPos($process.MainWindowHandle, [IntPtr](-2), 0, 0, 0, 0, 0x53) | Out-Null
