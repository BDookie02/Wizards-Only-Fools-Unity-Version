param(
    ...
    [int]$Height = 720,
    [ValidateSet('plains', 'jungle', 'swamp', 'tumbleweed')]
    [string]$Variant = 'plains'
)
...
if (-not $resolvedBuildRoot.StartsWith('D:\', [System.StringComparison]::OrdinalIgnoreCase) -or
    ...
    throw 'Detail-scatter probe paths must stay on D:.'
}
...
$profileRoot = Join-Path $resolvedOutputRoot ('detail-scatter-probe-profile-' + [Guid]::NewGuid().ToString('N'))
...
public static class WofDetailScatterCapture {
...
'@
[WofDetailScatterCapture]::SetProcessDPIAware() | Out-Null

$executable = Join-Path $resolvedBuildRoot 'WizardsOnlyFools.exe'
$logPath = Join-Path $logRoot "detail-scatter-${Variant}-probe.log"
$capturePath = Join-Path $resolvedOutputRoot "detail-scatter-${Variant}-${Width}x${Height}.png"
if (-not (Test-Path -LiteralPath $executable -PathType Leaf)) {
    throw "Windows player not found: $executable"
}
[System.IO.File]::WriteAllText(
    (Join-Path $profileRoot 'survival-save-v1.json'),
    '{"version":1,"playerName":"Detail Scatter QA","questUnlockedSpells":["blink"],"spellQuestAssignments":[],"questFlags":[]}')
...
$arguments = @(
    ...
    '--wof-solo', "--wof-detail-scatter-probe=$Variant", '--wof-sky-probe=day', '--wof-auto-exit=90',
...
try {
    ...
    do {
        ...
        $ready = (Test-Path -LiteralPath $logPath -PathType Leaf) -and
            (Select-String -LiteralPath $logPath -SimpleMatch 'DETAIL_SCATTER_PROBE_READY' -Quiet)
    } while (-not $ready -and -not $process.HasExited -and [DateTime]::UtcNow -lt $deadline)
    if (-not $ready) { throw "Detail-scatter $Variant probe did not become ready." }
    ...
    if ($windowHandle -eq [IntPtr]::Zero) { throw 'Detail-scatter player has no main window.' }
    [WofDetailScatterCapture]::ShowWindowAsync($windowHandle, 9) | Out-Null
    [WofDetailScatterCapture]::SetWindowPos($windowHandle, [IntPtr](-1), 0, 0, 0, 0, 0x53) | Out-Null
    [WofDetailScatterCapture]::SetForegroundWindow($windowHandle) | Out-Null

    $rect = New-Object WofDetailScatterCapture+RECT
    if (-not [WofDetailScatterCapture]::GetClientRect($windowHandle, [ref]$rect)) { throw 'GetClientRect failed.' }
    $point = New-Object WofDetailScatterCapture+POINT
    if (-not [WofDetailScatterCapture]::ClientToScreen($windowHandle, [ref]$point)) { throw 'ClientToScreen failed.' }
    ...
    [WofDetailScatterCapture]::SetWindowPos($windowHandle, [IntPtr](-2), 0, 0, 0, 0, 0x53) | Out-Null
    ...
    $runtimeFailure = Select-String -LiteralPath $logPath `
        ...
        -Quiet
    if ($runtimeFailure) { throw "Detail-scatter player logged a runtime exception: $logPath" }

    [PSCustomObject]@{
        Capture = $capturePath
        Log = $logPath
        Ready = (Select-String -LiteralPath $logPath -SimpleMatch 'DETAIL_SCATTER_PROBE_READY' |
    ...
    if (-not $process.HasExited) {
        [WofDetailScatterCapture]::SetWindowPos($process.MainWindowHandle, [IntPtr](-2), 0, 0, 0, 0, 0x53) | Out-Null
